const DocumentationEndpoint = (endpoint = '', method = '', route = '', contentEndpoint = []) => ({
...
DocumentationEndpoint.fromJSON = (json) => {
...
const DocumentationController = (controller = '', controllerRoute = '', endpointsCollection = []) => {
    if (!Array.isArray(endpointsCollection))
        throw Error('endpointsCollection deve ser um array de EndpointsCollection')
...
DocumentationController.fromJSON = (json) => {
    const endpoints = (json.EndpointsCollection || []).map(endpoint => DocumentationEndpoint.fromJSON(endpoint))
...
const APIDocumentation = (documentationControllers = []) => {
    if (!Array.isArray(documentationControllers))
        throw Error('documentationControllers deve ser um array de DocumentationController')

    return documentationControllers
}

APIDocumentation.fromJSON = (json) => {
    const parseAPI = JSON.parse(json)
    const controllers = parseAPI.map(controller => {
        if (controller)
            return DocumentationController.fromJSON(controller)
    })

    return APIDocumentation(controllers)
}
